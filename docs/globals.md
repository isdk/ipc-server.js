[**@isdk/ipc-server**](README.md) • **Docs**

***

# @isdk/ipc-server

## Enumerations

- [IPCClientStatus](enumerations/IPCClientStatus.md)
- [IPCErrorCode](enumerations/IPCErrorCode.md)
- [IPCMessageType](enumerations/IPCMessageType.md)

## Classes

- [IPCBaseConnection](classes/IPCBaseConnection.md)
- [IPCClient](classes/IPCClient.md)
- [IPCConnection](classes/IPCConnection.md)
- [IPCServer](classes/IPCServer.md)

## Interfaces

- [IPCClientOptions](interfaces/IPCClientOptions.md)
- [IPCPacketType](interfaces/IPCPacketType.md)
- [IPCServerOptions](interfaces/IPCServerOptions.md)

## Type Aliases

- [IPCPayloadData](type-aliases/IPCPayloadData.md)

## Variables

- [IPCEvents](variables/IPCEvents.md)
- [IPCNetServerEvents](variables/IPCNetServerEvents.md)
- [IPCNetSocketEvents](variables/IPCNetSocketEvents.md)

## Functions

- [wait](functions/wait.md)
